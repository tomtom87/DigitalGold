...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "digitalgold.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "digitalgoldd.pid"));
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 35 * 60) // Digitalgold: changed maximum adjust to 35 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Digitalgold will not work properly.");
